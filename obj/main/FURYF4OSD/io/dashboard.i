...
static void updateTicker(void)
{
    ...
    i2c_OLED_set_xy(dev, (128 / (5 + 1)) - 1, 0);
...
{
    i2c_OLED_set_xy(dev, (128 / (5 + 1)) - 2, 0);
    char rxStatus = '!';
...
static void updateFailsafeStatus(void)
{
    ...
    i2c_OLED_set_xy(dev, (128 / (5 + 1)) - 3, 0);
...
static void showRxPage(void)
{
    for (int channelIndex = 0; channelIndex < rxRuntimeState.channelCount && channelIndex < 14; channelIndex += 2) {
        i2c_OLED_set_line(dev, (channelIndex / 2) + 1);
...
static void showWelcomePage(void)
{
    ...
    tfp_sprintf(lineBuffer, "v%s (%s)", "4" "." "3" "." "3", shortGitRevision);
...
static void showGpsPage(void)
{
    ...
    i2c_OLED_set_xy(dev, __extension__ ({ __typeof__ (0) _a = (0); __typeof__ ((uint8_t)(((128 / (5 + 1)) - (sizeof(GPS_svinfo_cno) / sizeof(GPS_svinfo_cno[0]))) / 2)) _b = ((uint8_t)(((128 / (5 + 1)) - (sizeof(GPS_svinfo_cno) / sizeof(GPS_svinfo_cno[0]))) / 2)); _a > _b ? _a : _b; }), rowIndex++);
    ...
    for (index = 0; index < (sizeof(GPS_svinfo_cno) / sizeof(GPS_svinfo_cno[0])) && index < (128 / (5 + 1)); index++) {
        ...
        i2c_OLED_send_char(dev, (128 + 32) + bargraphOffset);
    }
    ...
    padHalfLineBuffer();
    i2c_OLED_set_xy(dev, ((128 / (5 + 1)) / 2), rowIndex++);
    i2c_OLED_send_string(dev, lineBuffer);
...
static void showDebugPage(void)
{
    for (int rowIndex = 0; rowIndex < 4; rowIndex++) {
        ...
        padLineBuffer();
        i2c_OLED_set_line(dev, rowIndex + 1);
        i2c_OLED_send_string(dev, lineBuffer);
    }
}
...
void dashboardUpdate(timeUs_t currentTimeUs)
{
    ...
    const 
# 656 "./src/main/io/dashboard.c" 3 4
         _Bool 
# 656 "./src/main/io/dashboard.c"
...
# 663 "./src/main/io/dashboard.c" 3 4
   _Bool 
# 663 "./src/main/io/dashboard.c"
...
# 664 "./src/main/io/dashboard.c" 3 4
   _Bool 
# 664 "./src/main/io/dashboard.c"
...
# 672 "./src/main/io/dashboard.c" 3 4
                                1
# 672 "./src/main/io/dashboard.c"
                                    ;
    } else {
        if (armedStateChanged) {
            pageState.pageFlags |= PAGE_STATE_FLAG_FORCE_PAGE_CHANGE;
...
void dashboardInit(void)
{
    ...
    dashBoardBus.busType_u.i2c.device = ((dashboardConfig()->device) - 1);
